...
func (g *generator) generateFile(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
	...
	for _, service := range file.Service {
		g.generateProtobufMetrics(file, service)
	}
...
func (g *generator) generateProtobufMetrics(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	svcName := serviceName(service) + "Metrics"
	...
	for _, method := range service.Method {
		...
		g.P("\t", methName, "ErrorCount metrics.Counter")
		g.P("\t", methName, "DurationNS metrics.Histogram")
	}
	g.P("}")
	g.P()
	...
	for _, method := range service.Method {
		methName := methodName(method)
		metricBase := strings.ToLower("twirp." + file.GetPackage() + "." + stringutils.SnakeCase(serviceName(service)) + "." + stringutils.SnakeCase(*method.Name))

		g.P("\tresult.", methName, "ErrorCount = prov.NewCounter(`", metricBase, ".error.count`)")
		g.P("\tresult.", methName, "DurationNS = prov.NewHistogram(`", metricBase, ".duration.ns`, 20)")
	}
	...
	for _, method := range service.Method {
		...
		g.P("\t\ti.", methName, "DurationNS.Observe(float64(time.Since(begin)))")
		g.P("\t\tif err != nil {")
		g.P("\t\t\ti.", methName, "ErrorCount.Add(1)")
		g.P("\t\t}")
		g.P("\t}(time.Now())")
		g.P()
